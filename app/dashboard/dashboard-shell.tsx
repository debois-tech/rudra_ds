...
export function DashboardShell({ profile, orgName, children }: DashboardShellProps) {
    ...
    return (
        <div className="min-h-screen flex flex-col bg-[#f8f9fb]">
            {/* ── Desktop Header — dark theme matching landing page ── */}
            <header className="hidden md:block sticky top-0 z-50 bg-[#0a0a0a] border-b border-white/10">
                <div className="flex h-14 items-center justify-between px-6 max-w-[1440px] mx-auto">
                    ...
                    <div className="relative" ref={userMenuRef}>
                        ...
                        {/* Dropdown */}
                        {userMenuOpen && (
                            <div className="absolute right-0 top-full mt-1.5 w-56 bg-[#141414] rounded-xl border border-white/10 shadow-lg shadow-black/50 py-1.5 animate-fade-in z-50">
                                <div className="px-3.5 py-2.5 border-b border-white/10">
                                    <p className="text-[13px] font-semibold text-white">{profile.full_name || 'User'}</p>
                                    <p className="text-[11px] text-gray-500 mt-0.5 truncate">{profile.email}</p>
                                </div>
                                <div className="p-1.5">
                                    <button
                                        onClick={handleSignOut}
                                        className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-[13px] font-medium text-red-400 hover:bg-red-500/10 transition-colors cursor-pointer"
                                    >
