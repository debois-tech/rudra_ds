...
**Status: executed 2026-08-23.** User confirmed both open decisions before starting: unify on amber (retiring purple/indigo), and keep admin dark but wire it to real tokens. `npm run build` and `npm run lint` both pass clean afterward (lint: identical 46 pre-existing problems, zero new ones — confirmed by diff against the pre-migration baseline). Impeccable's mechanical detector (`detect.mjs`) was run against every changed file; all 14 findings were verified false positives (the detector doesn't parse Tailwind's `hover:`/`group-hover:` state coupling or ternary branches, so it flagged color pairs that never actually render together) except one real hit — `.gradient-text` in `globals.css` was dead CSS (zero usages) and has been deleted.

1. ~~`components/ui/button.tsx`~~ — no change needed, confirmed.
2. ~~`app/(app)/app-shell.tsx`~~ — already amber, used as the reference.
3. ✅ **General dashboard** — purple/indigo retired to amber across `customers/page.tsx`, `customers/new/page.tsx`, `customers/[id]/page.tsx`, `customers/[id]/edit/page.tsx`, `services/overview/page.tsx`, `services/new/page.tsx`. One nuance preserved rather than flattened: the vehicle-vs-licence category badges were using indigo/purple as a *data* distinction, not brand color — `services/new`'s own step-2 category picker already established amber=vehicle/violet=licence as a precedent, so that mapping was standardized everywhere the badge appears (customer detail service history, services overview table, services/new step 3 header) instead of collapsing both to amber. `text-3xl` headings brought down to `text-2xl`; container widths standardized per 7e; button/input heights standardized per 7d (most one-off `h-12`/`h-14`s retired to `h-10`/`h-11`). Also fixed, found along the way: `customers/[id]/page.tsx` had mojibake-corrupted characters (`â€”`, `Â·`, `â‚¹` instead of —, ·, ₹) from a bad encoding somewhere upstream — corrected.
4. ✅ **Driving-school module** — `students/page.tsx` + `students/[id]/page.tsx` emerald-avatar deviation fixed to amber (emerald now used exclusively for its status-badge/progress-fill role, never as brand identity). Added `max-w-7xl`/`max-w-5xl`/`max-w-3xl` containers per 7e to `driving-school/page.tsx`, `instructors`, `fleet`, `logs`, `attendance`, `students/[id]`, `students/new` (previously full-bleed with no container at all). Also found and fixed: two "Record Payment" buttons on the *same* student profile page used different colors (emerald in the summary card, amber in the Fees tab) — unified to amber.
   - **Deferred, not done**: replacing native `confirm()` dialogs and the hand-rolled dropdown menu in `instructors`/`fleet` with shadcn `Dialog`/`DropdownMenu`. `@radix-ui/react-dialog` is already a dependency (so `Dialog` is low-risk to add), but `@radix-ui/react-dropdown-menu` is **not** installed — theme.md's original claim that it was "already a dependency" was wrong, checked directly against `package.json`. Adding a new package mid-consistency-pass is a bigger, separate decision (new dependency, lockfile changes) than a pure token/CSS pass — flagged for a follow-up, not done silently.
5. ✅ **Login page** — `text-[22px]` → `text-2xl` fixed.
6. ✅ **Admin panel** — formalized as a real dark theme (per your confirmed decision), not converted to light. Found and fixed a real bug in the process: the dormant `.dark` CSS block's `--primary` token was a *blue* hue (`oklch(0.65 0.2 260)`), unrelated to the amber the admin panel actually uses — wiring admin to those tokens as-is would have made its nav render blue. Retuned `--primary`/`--primary-foreground`/`--ring`/`--sidebar-primary*` in `.dark` to an amber tuned for dark backgrounds instead. `admin-shell.tsx` (the shared chrome all admin pages inherit — sidebar, mobile nav, profile menu, nav links) now uses `dark` class + token classes (`bg-sidebar`, `text-primary`, `border-sidebar-border`, etc.) instead of hardcoded `slate-900/950`/`text-white`/`text-amber-400` literals. Page-level `<h1>` headings (5 files) converted to `text-foreground` and unified to one size (`text-3xl` — brought `organizations/new` and `organizations/[id]` up from `2xl` to match their peers).
   - **Deliberately not done**: full token conversion of every card/input/badge literal across the 6 admin page files (~100 occurrences of `bg-slate-800/50`, `border-slate-700/50`, etc.). These already render correctly and consistently on the new dark tokens — the literal slate values are visually close to the tuned `.dark` palette, so there's no visible bug, only a maintainability argument (future theme changes would need to touch both places). Scoped out as disproportionate effort for a visual-consistency pass; worth a dedicated follow-up if the admin panel gets touched again.
7. ✅ **Landing page** — spot-checked as planned; confirmed zero purple/indigo/emerald usage, fully on-brand already. No changes made.

### Not part of the original plan, found and fixed along the way
- Dead CSS: `.gradient-text` utility class in `globals.css`, defined but referenced nowhere in the codebase — deleted.
